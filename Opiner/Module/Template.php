...
class Template extends \Opiner\Module
{
	...
	{
		$template = is_array($this -> settings) ? $this -> settings [0] : $this -> settings;
		$this -> load($template);
		if(isset($this -> settings ['meta']) and is_array($this -> settings ['meta']))
		foreach($this -> settings ['meta'] as $index => $value)
		{
			$this -> meta($index, $value);
			$this -> values ['site'] [$index] = $value;
		}
		if(isset($this -> settings ['meta'] ['title']))
		$this -> title($this -> settings ['meta'] ['title']);
		unset($this -> settings);
		return $this;
	}
